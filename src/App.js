...
const App = function() {
  ...
  const rotateY = function() {
    const toValue = index ? 0 : 1;
    setIndex(index ? 0 : 1);
    const options = {toValue, duration: 3000, useNativeDriver: false};
  ...
  const rotateX = function() {
    ...
    setIndex(index ? 0 : 1);
    const options = {toValue, duration: 3000, useNativeDriver: false};
    setAnimateStyle({transform: [{rotateX: interpolate}]});
    Animated.timing(animateValue, options).start();
  };
  ...
  const rotateZ = function() {
    ...
    setIndex(index ? 0 : 1);
    const options = {toValue, duration: 3000, useNativeDriver: false};
    setAnimateStyle({transform: [{rotateY: interpolate}]});
    Animated.timing(animateValue, options).start();
  };
